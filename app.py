...
DEFAULT_RASTER_DIR = "data/rasters"
...
def download_rasters_from_google_drive(folder_url: str, output_dir: str):
    """
    Downloads all files from a public Google Drive folder into output_dir.
    ...
    try:
        ...
        return True, "Google Drive rasters downloaded successfully."
    except Exception as e:
...
def read_raster_for_display(
    ...
    - selected_geojson_str is None: no clipping, raster displays exactly as exported.
    - selected_geojson_str exists: raster is clipped to selected district.
    """
...
# =========================================================
# Data Settings and Google Drive Download
# =========================================================

with st.expander("Data folder settings", expanded=False):
    c1, c2 = st.columns(2)

    with c1:
        raster_dir = st.text_input("Raster folder", DEFAULT_RASTER_DIR)

    with c2:
        boundary_dir = st.text_input("Boundary folder", DEFAULT_BOUNDARY_DIR)

    st.markdown("Google Drive raster folder")
    ...
    auto_download = st.checkbox(
        "Auto-download rasters from Google Drive if local raster folder is empty",
        value=True
    )

    if st.button("Download / Sync rasters from Google Drive"):
        with st.spinner("Downloading rasters from Google Drive. This may take time for large files..."):
            ok, msg = download_rasters_from_google_drive(drive_folder_url, raster_dir)
        if ok:
            st.success(msg)
        else:
            st.error(msg)
...
if auto_download and not local_rasters_exist(raster_dir):
    with st.spinner("No local rasters found. Downloading rasters from Google Drive..."):
        ok, msg = download_rasters_from_google_drive(drive_folder_url, raster_dir)
    if ok:
        st.success("Rasters downloaded from Google Drive.")
    else:
        st.error(msg)
...
if not rasters:
    st.error(
        f"No GeoTIFF rasters found in `{raster_dir}`. "
        "Either place .tif files inside data/rasters or check the Google Drive folder sharing."
    )
    st.stop()
